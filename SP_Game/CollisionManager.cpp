...
Parameters -> 1- walls (or anything that have colliding with object
			  ...
			  3- objects to check that every object(like as sheep or enemy and so on...)
...
//Testing 
void collisionManager2::applyCollisionWithAnimals(vector<Sprite>& walls, Vector2f& Velocity, RectangleShape& object)
{
	for (auto& wall : walls) {
		setObjectBound(object, wall);
		setNextBound(ob1Bounds, Velocity);
